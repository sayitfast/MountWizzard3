...
class MountModelHandling:
    ...
    def saveBackupModel(self):
        if self.saveModel('BACKUP'):
            if self.app.workerModelingDispatcher.modelingRunner.modelData:
                self.app.workerMountDispatcher.analyse.saveData(self.app.workerModelingDispatcher.modelingRunner.modelData, 'backup.dat')

    def loadBackupModel(self):
        if self.loadModel('BACKUP'):
            self.app.workerModelingDispatcher.modelingRunner.modelData = self.app.workerMountDispatcher.analyse.loadDataRaw('backup.dat')
    ...
    def saveBaseModel(self):
        if self.saveModel('BASE'):
            if self.app.workerModelingDispatcher.modelingRunner.modelData:
                self.app.workerMountDispatcher.analyse.saveData(self.app.workerModelingDispatcher.modelingRunner.modelData, 'base.dat')
            else:
                self.app.messageQueue.put('#BRNo data for BASE\n')

    def loadBaseModel(self):
        if self.loadModel('BASE'):
            self.app.workerModelingDispatcher.modelingRunner.modelData = self.app.workerMountDispatcher.analyse.loadDataRaw('base.dat')
            if not self.app.workerModelingDispatcher.modelingRunner.modelData:
    ...
    def saveRefinementModel(self):
        if self.saveModel('REFINE'):
            if self.app.workerModelingDispatcher.modelingRunner.modelData:
                self.app.workerMountDispatcher.analyse.saveData(self.app.workerModelingDispatcher.modelingRunner.modelData, 'refine.dat')
            else:
                self.app.messageQueue.put('#BRNo data for REFINE\n')

    def loadRefinementModel(self):
        if self.loadModel('REFINE'):
            self.app.workerModelingDispatcher.modelingRunner.modelData = self.app.workerMountDispatcher.analyse.loadDataRaw('refine.dat')
            if not self.app.workerModelingDispatcher.modelingRunner.modelData:
        ...
        if self.saveModel('ACTUAL'):
            if self.app.workerModelingDispatcher.modelingRunner.modelData:
                if 'Index' in self.app.workerModelingDispatcher.modelingRunner.modelData[0].keys():
                    self.app.workerMountDispatcher.analyse.saveData(self.app.workerModelingDispatcher.modelingRunner.modelData, 'actual.dat')
            else:
                self.app.messageQueue.put('#BRNo data for ACTUAL\n')

    def loadActualModel(self):
        if self.loadModel('ACTUAL'):
            self.app.workerModelingDispatcher.modelingRunner.modelData = self.app.workerMountDispatcher.analyse.loadDataRaw('actual.dat')
            if not self.app.workerModelingDispatcher.modelingRunner.modelData:
    ...
    def saveSimpleModel(self):
        if self.saveModel('SIMPLE'):
            if self.app.workerModelingDispatcher.modelingRunner.modelData:
                self.app.workerMountDispatcher.analyse.saveData(self.app.workerModelingDispatcher.modelingRunner.modelData, 'simple.dat')
            else:
                self.app.messageQueue.put('#BRNo data file for SIMPLE\n')

    def loadSimpleModel(self):
        if self.loadModel('SIMPLE'):
            self.app.workerModelingDispatcher.modelingRunner.modelData = self.app.workerMountDispatcher.analyse.loadDataRaw('simple.dat')
            if not self.app.workerModelingDispatcher.modelingRunner.modelData:
                self.app.messageQueue.put('#BRNo data file for SIMPLE\n')
    ...
    def saveDSO1Model(self):
        if self.saveModel('DSO1'):
            if self.app.workerModelingDispatcher.modelingRunner.modelData:
                self.app.workerMountDispatcher.analyse.saveData(self.app.workerModelingDispatcher.modelingRunner.modelData, 'DSO1.dat')
            else:
                self.app.messageQueue.put('#BRNo data file for DSO1\n')

    def loadDSO1Model(self):
        if self.loadModel('DSO1'):
            self.app.workerModelingDispatcher.modelingRunner.modelData = self.app.workerMountDispatcher.analyse.loadDataRaw('DSO1.dat')
            if not self.app.workerModelingDispatcher.modelingRunner.modelData:
                self.app.messageQueue.put('#BRNo data file for DSO1\n')
    ...
    def saveDSO2Model(self):
        if self.saveModel('DSO2'):
            if self.app.workerModelingDispatcher.modelingRunner.modelData:
                self.app.workerMountDispatcher.analyse.saveData(self.app.workerModelingDispatcher.modelingRunner.modelData, 'DSO2.dat')
            else:
                self.app.messageQueue.put('#BRNo data file for DSO2\n')

    def loadDSO2Model(self):
        if self.loadModel('DSO2'):
            self.app.workerModelingDispatcher.modelingRunner.modelData = self.app.workerMountDispatcher.analyse.loadDataRaw('dso2.dat')
            if not self.app.workerModelingDispatcher.modelingRunner.modelData:
                self.app.messageQueue.put('#BRNo data file for DSO2\n')

...
class Stick(QtCore.QThread):
    ...
    def run(self):
        ...
        while True:                                                                                                         # main loop for stick thread
            ...
            if self.connected:                                                                                              # differentiate between stick conected or not
                try:
                    data['DewPoint'] = self.ascom.DewPoint                                                                  # storing data in the signal object
                    data['Temperature'] = self.ascom.Temperature                                                            # actually there is singla based communication
                    data['Humidity'] = self.ascom.Humidity                                                                  # traget should be queue
                    ...
                    self.conected = False                                                                                   # if error occurs, set to disconnected
                ...
                try:
                    ...
                    self.logger.error('run -> connect win32com error: {0}'.fromat(e))                                       # write to logger

...
class indinumber(indielement):
    ...
    def get_number_of_steps(self):
        ...
        """
        if self.is_range():
            return int(math.floor(self.get_range()/self.get_step()))
        else:
            return 0
    ...
    def get_text(self):
        """
        @return: a formatted string representation of it value
        @rtype:  StringType
        """
...
class _blocking_indi_object_handler:
    ...
    def on_indiobject_changed(self, *args):
        ...
        B{Implement your custom handler here!!} (by overloading this function)
        @param args: The L{indiobject} (or indiobjects) that shall be used to update the GUI object
        @type args: list
        @return: B{None}
        @rtype:  NoneType
...
class indi_vector_identifier:
    """
    @ivar devicename : The name of the device containing the vector to be uniquely identified by this object
    @type devicename : StringType
    @ivar vectorname  : The name of the L{indivector} to be uniquely identified by this object
...
class indi_custom_element_handler(gui_indi_object_handler, indi_element_identifier):
    ...
    def configure(self, vector, element):
        """
        This handler method will be called only once. It will be called before the L{on_indiobject_changed} method is called for the
...
class indi_custom_vector_handler(gui_indi_object_handler, indi_vector_identifier):
    ...
    def on_indiobject_changed(self, vector):
        """
        This handler method will be called each time a specified INDI element/vector is received.
...
class bigindiclient:
    ...
    def _element_received(self, vector, element):
        ...
        for handler in self.custom_element_handler_list:
            if handler.vectorname == vector.name and handler.elementname == element.name and handler.devicename == vector.device:
    ...
    def _get_and_update_vector(self, attrs, tag):
        """
        Looks for an existing indivector matching the C{device} and C{name} attributes given in L{attrs}, and updates it
    ...
    def _get_and_update_element(self, attrs, tag):
        ...
        @rtype: NoneType
        """
        element = self.get_element(self.currentVector.device, self.currentVector.name, _normalize_whitespace(attrs.get('name', "")))
    ...
    def process_receive_vector_queue(self):
        ...
        """
        while not self.receive_vector_queue.empty():
    ...
    def add_custom_element_handler(self, handler):
        ...
        Furthermore this method will call the handler once. If the element has not been received yet, this function will wait
    ...
    def add_custom_vector_handler(self, handler):
        ...
        Furthermore this method will call the handler once. If the vector has not been received yet, this function will wait
    ...
    def process_events(self):
        ...
        self.process_receive_vector_queue()
        try:
            while not self.receive_event_queue.empty():
